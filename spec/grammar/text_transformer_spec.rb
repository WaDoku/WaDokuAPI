...
describe TextTransform do
  ...
  it "should transform complete entries" do
    ...
    res = transformer.apply(tree)
    res.should == "(N.) {Dom.: Bot.} Artischocke (f); (Scientif.: Cynara scolymus)."

    text = "(<POS: Adv. mit <Transcr.: to> und intrans. V. mit <Transcr.: suru>>) <MGr: <TrE: leuchtend grün>; <TrE: frisch>; <TrE: blass>>."
    tree = grammar.parse(text)
    res = transformer.apply(tree)
    res.should == "(Adv. mit to und intrans. V. mit suru) leuchtend grün; frisch; blass." 

    text = "(<POS: N.>) [1]<MGr: <TrE: grüne <HW f: Erde>>>. [2]<MGr: <TrE: grünes <HW n: Pigment>>>."
    tree = grammar.parse(text)
    res = transformer.apply(tree)
    res.should == "(N.) [1] grüne Erde (f). [2] grünes Pigment (n)."
    ...
    tree = grammar.parse(text)
    res = transformer.apply(tree)
    res.should == "{Dom.: Bsp.} Der Schüler übertrifft den Lehrer. (üblicher ist die Version: 'Ao wa ai yori idete ai yori aoshi. / 青は藍よりいでて藍より青し。')."
    ...
    tree = grammar.parse(text)
    res = transformer.apply(tree)
    res.should == "(N.) {Dom.: Boxen} Ecke (f) des Champions; Ecke (f) des Titelverteidigers. (⇒ 'ao·kōnā / 青コーナー')."
    ...
    tree = grammar.parse(text)
    res = transformer.apply(tree)
    res.should == "(N.) {Dom.: Kleidung} Pijacke (f); Kolani (m). (Def.: blaue Seemannsüberjacke; von engl. pea coat)."
    ...
    tree = grammar.parse(text)
    res = transformer.apply(tree)
    res.should == "(N.) {Dom.: Persönlichk.} Peter Michael BLAU. (Def.: amerik. Politiksoziologe österr. Abstammung; 1918–)."
    ...
    tree = grammar.parse(text)
    res = transformer.apply(tree)
    res.should == "(Adj.) [1] blau. grün (z. B. Ampel, Blattwerk). [2] blass; bleich. [3] unerfahren."
    ...
    tree = grammar.parse(text)
    res = transformer.apply(tree)
    res.should == "(N.) {Dom.: Werktitel} The Blue Boy (NAr); Der Knabe (NAr) in Blau. (Def.: Gemälde von Gainsborough; um 1770)."
    ...
    tree = grammar.parse(text)
    res = transformer.apply(tree)
    res.should == "(N.) [1] Doktor (m); Dr. (akad. Titel). [2] Arzt (m); Doktor (m). [3] Oberseminar (n); Graduiertenkurs (m); Doktorkurs (m) (Abk. für 'dokutā·kōsu / ドクター･コース'). (von engl. doctor)."
